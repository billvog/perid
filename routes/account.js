...
router.post('/login', auth.ensureNotAuthenticated, (req, res, next) => {
  ...
  passport.authenticate('local', (error, user, info) => {
    ...
    req.login(user, async (error) => {
      ...
      request(
        `http://api.ipstack.com/${IP}?access_key=${process.env.IPSTACK_API}`,
        {
          json: true,
        },
        (error, response, body) => {
          ...
          mailer.sendMail(
            {
              ...
              html: emailTemplates.AccountLoggedIn(
                user.firstName,
                body,
                DeviceInfo
              ),
      ...
      if (foundUser == null) {
        ...
        sendPasswordResetEmail(foundUser, (error, info) => {
          if (error) {
            ...
            res.render('account/forgot-password', {
              success_msg:
                'Password reset email has been sent (check spam folder)',
            });
          }
        });
      }
    ...
    sendChangePasswordEmail(req.user, (error, info) => {
      if (error) {
        ...
        res.render('account/change-password', {
          user: req.user,
          success_msg:
            'Change password email has been sent (check spam folder)',
        });
      }
    });
  }
...
router.get(
  '/my-account/create-new-password/:token',
  auth.ensureAuthenticated,
  (req, res) => {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const { user: id, type } = jwt.verify(token, process.env.JWT_SECRET);
        ...
        if (bcrypt.compareSync(password, req.user.password)) {
          return res.render('account/create-new-password', {
            ...
            errors: [
              {
                message:
                  'Please create a new password that differs from your old one',
              },
            ],
            // Input fields
            token,
  ...
  try {
    ...
    if (error.name == 'TokenExpiredError') {
      ...
      return res.render('account/verify-email', {
        user: req.user || undefined,
        errors: [
          { message: 'Invalid token. Please try sending the email again.' },
        ],
      });
    }
  }
    ...
    try {
      ...
      if (avatar) {
        if (avatar.size > maxAvatarSize) {
          errors.push({
            message: 'Avatar is too big, 5MB is the maximum size',
          });
        }
      ...
      if (
        new Date().getFullYear() -
          new Date(Date.parse(birthdate)).getFullYear() <=
        13
      ) {
        errors = [];
        errors.push({
          message:
            'To create an account to Perid, you must be over 13 years old.',
        });
      }
    ...
    try {
      ...
      if (req.user.email !== email) {
        ...
        mailer.sendMail(
          {
            from: 'Perid <no-reply@perid.tk>',
            to: [OldEmail, req.user.email],
            subject: "Your account's email has changed",
            html: emailTemplates.EmailHasChanged(
              req.user.firstName,
              OldEmail,
              req.user.email
            ),
      ...
      mailer.sendMail(
        {
          ...
          html: emailTemplates.AccountVerification(
            user.firstName,
            `https://perid.tk/account/verify/${token}`
          ),
        },
        (error, info) => {
          // console.log(info, error);
      ...
      mailer.sendMail(
        {
          ...
          html: emailTemplates.PasswordReset(
            user.firstName,
            `https://perid.tk/account/reset-password/${token}`
          ),
        },
        (error, info) => {
          // console.log(info, error);
      ...
      mailer.sendMail(
        {
          ...
          html: emailTemplates.PasswordChange(
            user.firstName,
            `https://perid.tk/account/my-account/create-new-password/${token}`
          ),
        },
        (error, info) => {
          // console.log(info, error);
